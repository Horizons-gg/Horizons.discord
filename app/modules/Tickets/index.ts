...
export class TicketController {
    ...
    async fetchController(channel: Discord.TextChannel): Promise<Discord.Message> {
        ...
        if (!controller?.content) throw new Error('Ticket Controller Not Found!')
        return controller
    }

    async fetchData(channel: Discord.TextChannel): Promise<[Ticket, Discord.Message]> {
    ...
    async setData(channel: Discord.TextChannel, options: Ticket): Promise<boolean> {
        const [data, controller] = await this.fetchData(channel)

        const content = `||${JSON.stringify({ ...data, ...options })}||`
        if (controller?.content === content) return true

        return await controller?.edit({ content })
            .then(() => true)
            .catch(() => { throw new Error('Failed to edit ticket data!') })
    }
    ...
    open(interaction: Discord.ButtonInteraction | Discord.ChatInputCommandInteraction): Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            Messages.send(channel, { description: `🔓 Ticket opened by ${interaction.user}`, color: 'success' })

            return this.setData(channel, { state: 'open' })
                .then(() => this.update(channel).then(() => resolve('Ticket Opened!')))
                .catch(reject)
        })
    }
    ...
    close(interaction: Discord.ButtonInteraction | Discord.ChatInputCommandInteraction): Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            Messages.send(channel, { description: `🔒 Ticket closed by ${interaction.user}`, color: 'danger' })

            return this.setData(channel, { state: 'closed' })
                .then(() => this.update(channel).then(() => resolve('Ticket Closed!')))
                .catch(reject)
        })
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            this.fetchData(channel)
                .then(([data]) => {
                    ...
                    Messages.send(channel, { description: `🛬 ${member} has been added to the ticket by ${interaction.user}`, color: 'info' })
        ...
        return new Promise((resolve, reject) => {
            ...
            this.fetchData(channel)
                .then(([data]) => {
                    ...
                    Messages.send(channel, { description: `🛫 ${member} has been removed from the ticket by ${interaction.user}`, color: 'info' })
    ...
    async archive(interaction: Discord.ButtonInteraction): Promise<Discord.TextChannel> {
        ...
        involvedStaff = involvedStaff.filter(id => {
            ...
            if (id === App.client.user?.id) return false
